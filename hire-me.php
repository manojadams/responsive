...
if(isset($_POST['event-complete'])&&is_user_logged_in()){
         ...
         $author_id = get_the_author_meta('ID');//returns the current posts author id

         if($author_id != $current_user->ID){//checking for authorization
            $hireme_error = TRUE;
            echo 'Error: Operation Not authorized';
            exit;
         }
        else $hireme_error = FALSE;
