...
class WPUF_Add_Post {
    ...
    function shortcode( $atts ) {
        ...
        extract( shortcode_atts( array('post_type' => 'post','wpuf_category' => ''), $atts ) );

        ob_start();

        if ( is_user_logged_in() ) {
            $this->post_form( $post_type,$wpuf_category );
        } else {
     ...
     * @param $post_type
     */
    function post_form( $post_type, $wpuf_category ) {
        global $userdata;
        ...
        if ( isset( $_POST['wpuf_post_new_submit'] ) ) {
            ...
            $this->submit_post($wpuf_category);
        }
                                ...
                                <div class="category-wrap" style="float:left;">
                                    <div id="lvl0">
										...
										<select name="category[]">
											<option selected="selected" value="<?php echo get_cat_ID($wpuf_category);?>"><?php echo $wpuf_category;?></option>
										</select>
                                    </div>
                                </div>
     ...
     */
    function submit_post($wpuf_category) {
        global $userdata;
        ...
        if ( wpuf_get_option( 'allow_cats', 'wpuf_frontend_posting', 'on' ) == 'on' ) {
            $post_category = isset($_POST['category'])?$_POST['category']:get_cat_ID($wpuf_category);
			
			
        } else {
            //$post_category = array(wpuf_get_option( 'default_cat', 'wpuf_frontend_posting' ));
			 $post_category = isset($wpuf_category)?array(get_cat_ID($wpuf_category)):array(wpuf_get_option( 'default_cat', 'wpuf_frontend_posting' ));
        ...
        if($post_category[0] == 117){//initialize custom field event-complete to 2
		    update_post_meta($post_id,'event-complete','2');
			}
        if ( $post_id ) {
           ...
           //echo '<div class="success">' . __('Post published successfully', 'wpuf') . '</div>';
            if ( $post_id ) {
               ...
               $musico->musico_AddHireMe($post_id,get_current_user_ID());
